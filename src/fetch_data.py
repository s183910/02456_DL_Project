...
def aa_fetch():
    ...
    if not os.path.isfile(raw_filepath):
        ...
        urllib.request.urlretrieve("https://ndownloader.figshare.com/files/1497002", raw_filepath)
...
def aa_parse(record_file):
    return tf.data.TFRecordDataset(record_file, compression_type="GZIP").map(aa_data_parse)
